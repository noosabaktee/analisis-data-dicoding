...
# Urutan favorit
def favorit(asc,y=False):
    # kondisi terhadap kolom shipping_limit_date
    # date = order_items[order_items.order_items >= ??]
...
def rating(asc):
    ...
    # Kondisi terhadap kolom review_answer_timestamp
...
def product_revenue(asc):
  df_income = order_items.merge(products,on="product_id")
  df_income = df_income[["shipping_limit_date","product_id","price","product_category_name"]]
  df_income = df_income.groupby("product_category_name").price.sum().reset_index()
  df_income = df_income.sort_values(by="price",ascending=asc)
  df_income.columns = ["category","revenue"]
